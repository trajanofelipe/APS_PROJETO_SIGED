...
@WebServlet("/LancaIndicador.do")
public class LancarIndicadorServlet extends HttpServlet {
...
//				Integer.parseInt(request.getParameter("valorIndicador").trim()); // campo obrigatório		
		...
		String result = IndicadorManager.lancarIndicador( idIndicador, dataIndicador,  nomeIndicador, depIndicador, metaIndicador, valorIndicador );		
		request.setAttribute("mensagem", result);
		RequestDispatcher view = request.getRequestDispatcher("lancarIndicador.jsp");
		...
		if( result.contains("Não foi possível lançar")) {
			...
			view = request.getRequestDispatcher("lancarIndicador.jsp");
		}		
